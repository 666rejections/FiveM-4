...
ESX.Game.Teleport = function(entity, coords, cb)
  ...
  TriggerServerEvent('logs:write', "Est tp aux coordonnés : "..tostring(json.encode(coords)))
...
ESX.Game.GetClosestPed = function(pos, ignoreList)

  local ignoreList      = ignoreList or {}
  local peds            = ESX.Game.GetPeds(ignoreList)
  local closestDistance = -1
  local closestPed      = -1
  local coords = pos or GetEntityCoords(GetPlayerPed(-1), false)
...
ESX.ShowInventory = function()
  ...
  for i=1, #ESX.PlayerData.inventory, 1 do
    ...
    if ESX.PlayerData.inventory[i].count > 0 then
    	if not IsFoodItem(ESX.PlayerData.inventory[i].name) then
			table.insert(elements, {
				label     = ESX.PlayerData.inventory[i].label .. ' x'..ESX.PlayerData.inventory[i].count..' - ' .. ESX.PlayerData.inventory[i].limit*ESX.PlayerData.inventory[i].count..'g',
			...
			totalWeight = totalWeight + ESX.PlayerData.inventory[i].limit*ESX.PlayerData.inventory[i].count
		else
			table.insert(tempFood, {
				label     = ESX.PlayerData.inventory[i].label .. ' x'..ESX.PlayerData.inventory[i].count..' - ' .. ESX.PlayerData.inventory[i].limit*ESX.PlayerData.inventory[i].count..'g',
				count     = ESX.PlayerData.inventory[i].count,
				type      = 'item_standard',
				value     = ESX.PlayerData.inventory[i].name,
			...
			foodWeight = foodWeight + ESX.PlayerData.inventory[i].limit*ESX.PlayerData.inventory[i].count
		end
    end
  end
	table.insert(elements, {label = '<span style="color:green;">= UTILITAIRE - '..tostring(foodWeight/1000)..'/'..tostring(Config.MaxFoodInventoryWeight/1000)..'kg =</span>'})
	for i=1, #tempFood, 1 do
		table.insert(elements, tempFood[i])
	end
  table.insert(elements, {label = '<span style="color:red;">======== ARMES ========</span>'})
  for i=1, #Config.Weapons, 1 do
    ...
    if HasPedGotWeapon(playerPed,  weaponHash,  false) and Config.Weapons[i].name ~= 'WEAPON_UNARMED' then
      ...
      table.insert(elements, {
        ...
        count     = 1,
        weight    = Config.Weapons[i].weight,
        type      = 'item_weapon',
  ...
  ESX.UI.Menu.Open(
    ...
    {
      title    = 'Inventaire '..tostring(totalWeight/1000)..'/'..tostring(Config.MaxInventoryWeight/1000)..'kg',
      elements = elements,
    },
    function(data, menu)
                  ...
                  if pedammo > 0 then
                    ...
                    function(data2, menu2)
                      ...
                      if quantity <= pedammo and quantity >= 0 and quantity ~= nil then
                        local ammobefore = GetAmmoInPedWeapon(playerPed, GetHashKey(item))
                        TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), type, item, quantity, totalWeight)
                    ...
                    TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(closestPlayer), type, item, 0, totalWeight)
                    menu.close()
                  end
                else
              ...
              ESX.UI.Menu.Open(
                ...
                function(data2, menu2)
                  ...
                  if quantity == nil then
                    ...
                    TriggerServerEvent('esx:removeInventoryItem', type, item, quantity)
  					TriggerServerEvent('logs:write', 'A jeté '..tostring(quantity)..' '..item)
                  end
