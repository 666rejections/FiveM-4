...
local function StartVehicleManagment()
	...
	while true do
		...
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1) then
				...
				if vehiclesEngines[i] ~= nil then
					if vehiclesEngines[i].plate == GetVehicleNumberPlateText(vehicle) then
						if vehiclesEngines[i].engine then
							SetVehicleEngineOn(vehicle, not vehiclesEngines[i].engine, true, true)
						end
						break
					end
				end
...
AddEventHandler('vehicleManager:startVehicleByKit', function(engines)
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		...
		local WaitingTime = math.random(3000, 10000)
...
local function StartShowSpeed()
	...
	while true do
		...
		if IsPedInAnyVehicle(ped, true) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped, false), -1) == ped then
			if IsControlPressed(1, 97) then
				...
				elseif IsControlJustPressed(1, 201) then --ACCEPT
					SetEntityMaxSpeed(vehicle, (maxSpeed-1)/3.6)
		...
		if IsPedSittingInAnyVehicle(ped) then
			local speed = math.floor(GetEntitySpeed(GetVehiclePedIsIn(ped, false))*3.6)
			if speed >= 151 then
...
AddEventHandler('vehicleManager:getVehiclesKey', function()
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		TriggerEvent('esx_key:getVehiclesKey', GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1), true)))
	else
		local vehicle, distance = ESX.Game.GetClosestVehicle()
		if distance < 10.0 then
			TriggerEvent('esx_key:getVehiclesKey', GetVehicleNumberPlateText(vehicle))
		else
			TriggerEvent('esx:showNotification', "~r~Aucun véhicule détecté")
		end
	end
end)